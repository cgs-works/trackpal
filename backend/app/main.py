...
from app.services.lookup_execution_coordinator import (
...
class PublicCatalogCORS(CORSMiddleware):
    ...
    This subclass bypasses CORS processing for the public catalog path so the
    endpoint retains full control of its dynamic response headers.
    """
...
async def lifespan(app: FastAPI):
    global _cleanup_task, _export_worker_task, _export_cleanup_task
    ...
    if manager is None:
        ...
        configure_lookup_execution_coordinator(None)
    else:
        coordination_store = RedisLookupCoordinationStore(manager)
        configure_active_lease_reader(coordination_store)
        configure_lookup_execution_coordinator(
            LookupExecutionCoordinator(
                AsyncSessionLocal,
                coordination_store,
    ...
    tasks = [
        task
        for task in (_cleanup_task, _export_worker_task, _export_cleanup_task)
        if task is not None
    ]
    for task in tasks:
        task.cancel()
