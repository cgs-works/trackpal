...
async def _get_mode(manager: object, phone: str) -> str | None:
    key = f"{MODE_KEY_PREFIX}{phone}"

    async def _get(client: object) -> str | None:
...
async def _set_mode(manager: object, phone: str, mode: str) -> None:
    key = f"{MODE_KEY_PREFIX}{phone}"

    async def _set(client: object) -> None:
        await client.set(key, mode, ex=MODE_TTL)

    await manager.execute("set_mode", _set)


async def _clear_mode(manager: object, phone: str) -> None:
    key = f"{MODE_KEY_PREFIX}{phone}"

    async def _del(client: object) -> None:
...
async def _handle_ambiguity(
    ...
    if msg_lower in ("menu", "/menu"):
        await _clear_mode(manager, phone)
        return WhatsAppConsoleResponse(reply=t(locale, "wa.client.mode_reset"))

    stored_mode = await _get_mode(manager, phone)
    if stored_mode == "tenant":
        return await _handle_tenant_console(
            phone=phone, message=message, instance=instance, manager=manager, db=db,
        )
...
def _client_identity(client: object, tenant: object) -> dict[str, str]:
    return {
