...
Provides three Redis primitives keyed by sender phone (canonical
digits-only):

*   ``wa:auth:{phone}`` — authenticated session with user metadata, TTL
    set to the WhatsApp session TTL (default 15 minutes).
*   ``wa:auth:fail:{phone}`` — consecutive failure counter with
    timestamps, TTL = failure window (default 15 minutes).
*   ``wa:auth:lock:{phone}`` — temporary lockout marker, TTL = lock
    duration (default 5 minutes).

No password is ever stored in any payload.
"""
    ...
    async def record_failed_attempt(
        ...
        *lock_state* is the ``WhatsAppAuthLockState`` when the phone
        ...
        async def _record(client: Any) -> tuple[int, WhatsAppAuthLockState | None]:
            ...
            if state is None:
                # First failure — create fresh fail state
