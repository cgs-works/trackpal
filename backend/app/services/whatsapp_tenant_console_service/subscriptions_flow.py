...
async def _query_subscriptions_by_filter(self, db, tenant_id, filter_msg):
    ...
    if filter_msg == "1":
        return await self._subscription_service.list_subscriptions(
...
async def _handle_subscriptions_filter(
    ...
    if not subscriptions:
        return (
            self._t(self.KEY_SUBSCRIPTIONS_NO_RESULTS)
            + "\n\n"
    ...
    reply, selection_map = self._format_subscription_list(
...
async def _handle_subscriptions_action(
    ...
    if msg == "2":
        ...
        return self._t(
            self.KEY_SUBSCRIPTIONS_CANCEL_CONFIRM_TEMPLATE,
...
async def _apply_subscription_update(
    self, phone, session, session_service, tenant_id, db, **changes
):
    if tenant_id is None or db is None or self._subscription_service is None:
    ...
    normalized_changes = {
        key: (None if value == "" else value) for key, value in changes.items()
    }
    try:
    ...
    if session_service is not None:
        await session_service.clear_session(f"admin:{phone}")
    return (
        self._with_main_menu(self._t(self.KEY_SUBSCRIPTIONS_EDIT_SUCCESS))
