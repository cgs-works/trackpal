...
async def _route_profile_flow(
    self, phone, msg, step, session, session_service, user_id, db
):
...
async def _route_subscriptions_flow(
    self, phone, msg, step, session, session_service, tenant_id, db
):
    create_steps = {
    ...
    if step == self.SUBSCRIPTIONS_STEP_MENU:
        ...
        return await _route_subscriptions_create_flow(
            self, phone, msg, step, session, session_service, tenant_id, db
        )
    elif step in edit_steps:
        return await _route_subscriptions_edit_flow(
            self, phone, msg, step, session, session_service, tenant_id, db
        )
    elif step in lifecycle_steps:
        return await _route_subscriptions_lifecycle_flow(
            self, phone, msg, step, session, session_service, tenant_id, db
        )
    return self._t(self.KEY_FALLBACK_ACTIVE_FLOW)


async def _route_subscriptions_create_flow(
    self, phone, msg, step, session, session_service, tenant_id, db
):
...
async def _route_subscriptions_edit_flow(
    self, phone, msg, step, session, session_service, tenant_id, db
):
...
async def _route_subscriptions_lifecycle_flow(
    self, phone, msg, step, session, session_service, tenant_id, db
):
