"""Tests for the Client Messaging Block repository."""
...
from app.repositories import client_messaging_block_repository as cmb_repo
...
class TestCreate:
    async def test_create_with_phone(self, db_session: AsyncSession) -> None:
        ...
        block = await cmb_repo.create(db_session, tenant.id, phone="12015550030")
    ...
    async def test_create_with_lid(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        block = await cmb_repo.create(db_session, tenant.id, whatsapp_lid="test@lid")
    ...
    async def test_create_requires_identity(self, db_session: AsyncSession) -> None:
        ...
        with pytest.raises(ValueError, match="identity field"):
            await cmb_repo.create(db_session, tenant.id)


class TestFindActive:
    async def test_find_by_phone(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        found = await cmb_repo.find_active(db_session, tenant.id, phone="12015550030")
        assert found is not None
        assert found.phone == "12015550030"

    async def test_find_by_lid(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        await cmb_repo.create(db_session, tenant.id, whatsapp_lid="blocked@lid")
        found = await cmb_repo.find_active(
    ...
    async def test_find_returns_none_for_no_match(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        found = await cmb_repo.find_active(db_session, tenant.id, phone="12015559999")
        assert found is None

    async def test_find_returns_none_when_unblocked(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        block = await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        await cmb_repo.unblock(db_session, tenant.id, block.id)
        found = await cmb_repo.find_active(db_session, tenant.id, phone="12015550030")
        assert found is None

    async def test_find_no_args_returns_none(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        assert await cmb_repo.find_active(db_session, tenant.id) is None
...
class TestTenantIsolation:
    async def test_blocks_are_tenant_scoped(self, db_session: AsyncSession) -> None:
        ...
        await cmb_repo.create(db_session, tenant_a.id, phone="12015550030")
        found = await cmb_repo.find_active(db_session, tenant_b.id, phone="12015550030")
        assert found is None
    ...
    async def test_list_active_is_tenant_scoped(self, db_session: AsyncSession) -> None:
        ...
        tenant_b = await _seed_other_tenant(db_session)
        await cmb_repo.create(db_session, tenant_a.id, phone="12015550030")
        await cmb_repo.create(db_session, tenant_a.id, phone="12015550031")
        blocks_a = await cmb_repo.list_active(db_session, tenant_a.id)
        blocks_b = await cmb_repo.list_active(db_session, tenant_b.id)
    ...
    async def test_unblock_is_tenant_scoped(self, db_session: AsyncSession) -> None:
        _user, tenant_a = await _seed_tenant(db_session)
        tenant_b = await _seed_other_tenant(db_session)
        block = await cmb_repo.create(db_session, tenant_a.id, phone="12015550030")
        # Attempt to unblock from wrong tenant
        result = await cmb_repo.unblock(db_session, tenant_b.id, block.id)
        assert result is None
        # Block still active in original tenant
        found = await cmb_repo.find_active(db_session, tenant_a.id, phone="12015550030")
        assert found is not None
    ...
    async def test_clear_identity_is_tenant_scoped(self, db_session: AsyncSession) -> None:
        ...
        cleared = await cmb_repo.clear_identity(db_session, tenant_b.id, phone="12015550030")
        assert cleared == 0
        # Block still active in original tenant
        found = await cmb_repo.find_active(db_session, tenant_a.id, phone="12015550030")
        assert found is not None
...
class TestListActive:
    async def test_list_empty(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        blocks = await cmb_repo.list_active(db_session, tenant.id)
        assert blocks == []

    async def test_list_returns_only_active(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        block_a = await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        await cmb_repo.create(db_session, tenant.id, phone="12015550031")
        await cmb_repo.unblock(db_session, tenant.id, block_a.id)
        blocks = await cmb_repo.list_active(db_session, tenant.id)
        assert len(blocks) == 1
    ...
    async def test_list_orders_newest_first(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        block_a = await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        block_b = await cmb_repo.create(db_session, tenant.id, phone="12015550031")
        blocks = await cmb_repo.list_active(db_session, tenant.id)
...
class TestUnblock:
    async def test_unblock_active_block(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        block = await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        result = await cmb_repo.unblock(db_session, tenant.id, block.id)
    ...
    async def test_unblock_nonexistent_returns_none(
        ...
        _user, tenant = await _seed_tenant(db_session)
        import uuid
        result = await cmb_repo.unblock(db_session, tenant.id, uuid.uuid4())
        assert result is None
...
class TestClearIdentity:
    async def test_clear_by_phone(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        cleared = await cmb_repo.clear_identity(db_session, tenant.id, phone="12015550030")
        assert cleared == 1
        found = await cmb_repo.find_active(db_session, tenant.id, phone="12015550030")
        assert found is None
    ...
    async def test_clear_by_lid(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        await cmb_repo.create(db_session, tenant.id, whatsapp_lid="blocked@lid")
        cleared = await cmb_repo.clear_identity(
            db_session, tenant.id, whatsapp_lid="blocked@lid"
        )
        assert cleared == 1
        found = await cmb_repo.find_active(
    ...
    async def test_clear_no_args_returns_zero(
        ...
        _user, tenant = await _seed_tenant(db_session)
        assert await cmb_repo.clear_identity(db_session, tenant.id) == 0

    async def test_clear_no_match_returns_zero(
        self, db_session: AsyncSession
    ) -> None:
        _user, tenant = await _seed_tenant(db_session)
        assert await cmb_repo.clear_identity(db_session, tenant.id, phone="12015559999") == 0

    async def test_clear_idempotent(self, db_session: AsyncSession) -> None:
        _user, tenant = await _seed_tenant(db_session)
        await cmb_repo.create(db_session, tenant.id, phone="12015550030")
        await cmb_repo.clear_identity(db_session, tenant.id, phone="12015550030")
        cleared = await cmb_repo.clear_identity(db_session, tenant.id, phone="12015550030")
        assert cleared == 0
...
class TestPersistence:
    ...
    async def test_block_persists_across_sessions(
        ...
        _user, tenant = await _seed_tenant(db_session)
        tenant_id = tenant.id
        block = await cmb_repo.create(db_session, tenant_id, phone="12015550030")
        await db_session.commit()

        # Re-fetch from a "new" session perspective
        db_session.expire_all()
        found = await cmb_repo.find_active(db_session, tenant_id, phone="12015550030")
        assert found is not None
    ...
    async def test_block_persists_until_unblocked(
        ...
        await cmb_repo.unblock(db_session, tenant_id, block.id)
        await db_session.commit()
    ...
    async def test_created_block_appears_in_list(
        ...
        _user, tenant = await _seed_tenant(db_session)
        tenant_id = tenant.id
        await cmb_repo.create(db_session, tenant_id, phone="12015550030")
        await db_session.commit()
        db_session.expire_all()
        blocks = await cmb_repo.list_active(db_session, tenant_id)
        assert len(blocks) == 1
...
class TestClientCreationClearsBlock:
    ...
    async def test_clear_on_client_creation_by_phone(
        ...
        # Assert
        assert cleared == 1
        remaining = await cmb_repo.list_active(db_session, tenant.id)
    ...
    async def test_clear_on_client_creation_by_lid(
        ...
        await cmb_repo.create(db_session, tenant.id, whatsapp_lid="newclient@lid")
        await db_session.flush()
