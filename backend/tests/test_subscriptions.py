...
async def test_subscription_api_rejects_plan_service_mismatch(
    ...
    assert (
        "plan not found" in response.json()["detail"].lower()
        or "plan no encontrado" in response.json()["detail"].lower()
    )
...
async def test_subscription_api_settings_update_persists(client, active_tenant_user):
    ...
    response = await client.put(
        "/api/v1/subscription-settings", json=update_payload, headers=headers
    )
...
@pytest.mark.asyncio
async def test_subscription_api_settings_accepts_both_recipient_mode(
    client, active_tenant_user
):
...
async def test_subscription_reactivate_userfacing_error_translated(
    ...
    assert (
        "Failed to reactivate subscription" in detail
        or "No se pudo reactivar" in detail
    )
...
async def test_reminder_payload_not_persisted_when_render_fails(
    ...
    assert resp_pending.status_code == 200
    data = resp_pending.json()
    assert data["items"] == [], "Expected no items when message rendering fails"
    ...
    assert len(rows) == 0, f"Expected 0 logs when render fails, got {len(rows)}"
...
async def test_reminder_payload_not_persisted_when_decrypt_fails(
    ...
    assert resp_pending.status_code == 200
    data = resp_pending.json()
    assert data["items"] == [], "Expected no items when decrypt fails"
    ...
    rows = (await db_session.execute(stmt)).scalars().all()
    assert len(rows) == 0, f"Expected 0 logs when decrypt fails, got {len(rows)}"
...
async def test_reminder_duplicate_log_skipped_gracefully(
    ...
    assert resp_pending.status_code == 200
    data = resp_pending.json()
    assert data["items"] == [], "Expected no items when flush raises IntegrityError"
...
async def test_reminder_unexpected_flush_error_logged(
    ...
    assert resp_pending.status_code == 200
    data = resp_pending.json()
    assert data["items"] == [], "Expected no items when unexpected flush error occurs"
