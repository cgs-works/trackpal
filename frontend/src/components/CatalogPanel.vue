...
<template>
  <div class="rounded-xl border border-border bg-card shadow-sm">
    ...
    <div class="px-6 py-4 border-b border-border">
      <form @submit.prevent="createService" class="flex flex-wrap items-end gap-3">
        <div class="flex flex-col gap-1.5 min-w-[200px]">
          <label class="text-xs font-medium text-muted-foreground">{{ i18nStore.t('frontend.catalog.new_service') }}</label>
          <Input v-model.trim="serviceName" type="text" required />
        </div>
        <Button type="submit" variant="default" size="sm">{{ i18nStore.t('frontend.catalog.create_service') }}</Button>
      </form>
    </div>
    ...
    <div class="px-6 py-4 border-b border-border">
      ...
      <div v-else class="flex flex-col gap-1.5">
        <div
          ...
          :class="{ 'border-indigo-300 dark:border-indigo-700 bg-indigo-50/50 dark:bg-indigo-950/20': selectedServiceId === service.id }"
        >
          <button
            type="button"
          ...
          <div class="flex items-center gap-1">
            <Button variant="ghost" size="sm" @click="openRenameService(service)">{{ i18nStore.t('frontend.catalog.edit') }}</Button>
            <Button variant="ghost" size="sm" class="text-destructive hover:text-destructive" @click="openDeleteService(service)">{{ i18nStore.t('frontend.catalog.delete') }}</Button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedServiceId" class="px-6 py-4 border-b border-border">
      <form @submit.prevent="createPlan" class="flex flex-wrap items-end gap-3">
        <div class="flex flex-col gap-1.5 min-w-[200px]">
          <label class="text-xs font-medium text-muted-foreground">{{ i18nStore.t('frontend.catalog.new_plan') }}</label>
          <Input v-model.trim="planName" type="text" required />
        </div>
        <Button type="submit" variant="default" size="sm">{{ i18nStore.t('frontend.catalog.create_plan') }}</Button>
      </form>
    </div>
    ...
    <div v-if="selectedServiceId" class="px-6 py-4">
      ...
      <div v-else class="flex flex-col gap-1.5">
        ...
        >
          ...
          <div class="flex items-center gap-1">
            <Button variant="ghost" size="sm" @click="openRenamePlan(plan)">{{ i18nStore.t('frontend.catalog.edit') }}</Button>
            <Button variant="ghost" size="sm" class="text-destructive hover:text-destructive" @click="openDeletePlan(plan)">{{ i18nStore.t('frontend.catalog.delete') }}</Button>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="space-y-3">
          ...
          <template v-else-if="deletePreview">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <Input v-model.trim="deleteConfirmText" type="text" :placeholder="i18nStore.t('frontend.catalog.confirm_placeholder')" />
            </div>
          </template>
        </div>
