...
<template>
  <Dialog :open="isOpen" @update:open="(v) => !v && close()">
    <DialogContent class="sm:max-w-lg">
      ...
      <p v-if="errorMessage" class="text-sm text-red-500 bg-red-50 dark:bg-red-950/20 border border-red-200/30 dark:border-red-950/40 rounded px-3 py-2">{{ errorMessage }}</p>
      ...
      <p v-else-if="loadError" class="text-sm text-red-500 bg-red-50 dark:bg-red-950/20 border border-red-200/30 dark:border-red-950/40 rounded px-3 py-2">{{ loadError }}</p>
      ...
      <div v-if="!isLoading && !loadError" class="flex flex-col gap-4">
        <div class="flex items-center justify-between gap-3">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            ...
            <div class="w-11 h-6 bg-muted-foreground/30 rounded-full peer peer-checked:bg-primary peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
          </label>
        </div>
        ...
        <template v-if="settings.reminders_enabled">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div v-if="settings.warning_days.length" class="flex flex-wrap gap-1.5 mt-1">
              <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 text-xs font-semibold" v-for="day in settings.warning_days" :key="day">
                {{ day }} {{ day === 1 ? i18nStore.t('frontend.subscriptions.day') : i18nStore.t('frontend.subscriptions.day') + 's' }}
