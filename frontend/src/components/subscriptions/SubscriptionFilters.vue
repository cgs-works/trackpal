...
<template>
  <div class="bg-card border border-border rounded-md p-5 mb-6 shadow-sm">
    <div class="flex items-center justify-between border-b border-border pb-3 mb-4">
      <div>
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('frontend.subscriptions.filters') }}</span>
        <h2 class="text-sm font-bold text-foreground mt-0.5">{{ t('frontend.subscriptions.search') }}</h2>
      </div>
      <button
        @click="clearFilters"
        type="button"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-semibold text-muted-foreground">{{ t('frontend.subscriptions.status') }}</label>
        <select
          v-model="filters.status"
          class="px-3 py-2 text-xs bg-background border border-border rounded-md text-foreground focus:outline-none focus:border-primary transition-all cursor-pointer"
        >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 border-t border-border pt-4">
      <div class="flex flex-wrap gap-2 items-center">
        <span class="text-xs font-semibold text-muted-foreground mr-2">{{ t('frontend.subscriptions.status') }}:</span>
        <button
          @click="setQuickFilter('this_week')"
          type="button"
          class="px-2.5 py-1 text-xs bg-secondary hover:hover:bg-accent text-foreground rounded font-medium transition-all cursor-pointer"
        >
      ...
      <div class="flex items-center gap-2 md:justify-end">
        <div class="flex items-center gap-1.5">
          <label class="text-[10px] font-bold text-muted-foreground uppercase">{{ t('frontend.subscriptions.filter_from') }}</label>
          <input
            v-model="filters.expires_from"
            type="date"
            class="px-2 py-1 text-xs bg-background border border-border rounded text-foreground focus:outline-none"
          />
        </div>
        <span class="text-muted-foreground">—</span>
        <div class="flex items-center gap-1.5">
          <label class="text-[10px] font-bold text-muted-foreground uppercase">{{ t('frontend.subscriptions.filter_to') }}</label>
          <input
        ...
        <button
          @click="applyFilters"
          type="button"
          class="px-3 py-1 bg-primary hover:bg-primary/90 text-primary-foreground rounded text-xs font-semibold cursor-pointer transition-colors ml-2"
        >
