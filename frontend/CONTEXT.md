...
| **My Account** | User-facing name for the selected Tenant business profile and data surfaces (`Mi cuenta` / `My account`). Tenant Admin sees Profile, Security, Data, and — when Pro — the Regional Tab; Master Support Context uses the same label for the selected business but omits Security and self-service deletion. It never refers to the authenticated Master's own identity; keep **Tenant** as the internal domain term and avoid exposing that jargon in UI copy. |
| **Regional Tab** | Pestaña de Mi Cuenta (UI: `País y moneda` / `Country & currency`) donde el Tenant Admin elige su **Country** y **Currency**. El selector de Currency muestra primero la **Official Currency** del Country seleccionado, separada del resto; cambiar de Country nunca sobrescribe la Currency elegida. Es Pro-only: Starter la oculta (patrón timezone) y Master en contexto de soporte la ve. |
