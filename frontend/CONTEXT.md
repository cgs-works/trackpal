...
| **My Account** | User-facing name for the selected Tenant business profile and data surfaces (`Mi cuenta` / `My account`). Tenant Admin sees Profile, Security, Regional, and Data tabs; Master Support Context uses the same label for the selected business but omits Security and self-service deletion. It never refers to the authenticated Master's own identity; keep **Tenant** as the internal domain term and avoid exposing that jargon in UI copy. |
| **Regional Tab** | Pestaña "Configuración regional" / "Regional settings" dentro de My Account. Agrupa Country, Idioma, Zona horaria y Currency. Para Starter tenant admin solo muestra Country e Idioma (zona horaria y moneda ocultas); Master en contexto de soporte ve los 4. |
| **CountryPicker** | Selector de países del Currency Catalog. Guarda códigos ISO 3166-1 alpha-2 y localiza los nombres en el browser con `Intl.DisplayNames` según el locale activo. Disponible para Starter y Pro. |
| **CurrencyPicker** | Selector de monedas del Currency Catalog con símbolo. Si el Tenant tiene Country elegido, su Official Currency aparece primero en un grupo separado y luego un separador con el resto de monedas; sin Country, lista plana. Nunca sobreescribe la Currency guardada al cambiar el Country. Pro-only. |
...
| **Settings Store** | Cache de configuración de tenant, recordatorios, timezones, opciones de moneda/país (currency options), mailbox y Public API Key. |
| **Gmail Setup Assistant** | The sole Settings experience for connecting the Central Lookup Mailbox. It guides the Tenant Admin to create a Google app password at myaccount.google.com/apppasswords, then collects the Gmail address and app password. Avoid exposing **IMAP**, server, port, or SSL terminology. |
