...
- `ensure()` creates the envelope lazily after authenticated Demo context exists; Starter initializes the Master-provided business name, the Master-selected `en`/`es` locale, connected simulated mailbox/WhatsApp states, three ordered generic code services, and two ordered blocked identities. Pro subscription records may carry only fictitious demo secret values for the local reveal flow; they are never sent to production services or logged. `reset()` restores that baseline locale while preserving tour state, and `clear()` removes only the selected Demo Account's workspace.
