...
Each view wraps its content in `DashboardLayout`, which provides the dark-only sidebar shell + mobile nav + locale controls.
...
- **Desktop sidebar** (hidden on mobile): compact Trackpal branding, navigation links, exit-support button (support mode only), locale selector, user card + logout
...
- **Dark-only UI contract**: no theme toggle and no light-mode surfaces; `src/lib/darkTheme.js` forces the root `.dark` class during bootstrap.
...
These replace ad-hoc card, button, input, and table markup across views. Shared command-center components (`SummaryMetric`, `EntityInspector`, `ImpactConfirmDialog`) compose these primitives for summary-first workspaces, selected-row inspection, and destructive confirmations.
...
- Dark-only command-center scheme forced by `src/lib/darkTheme.js` before app mount
- **shadcn-vue** CSS variables pattern: `--background`, `--foreground`, `--card`, `--muted`, `--border`, `--primary`, `--destructive`, plus command-center surface tokens (`--surface-raised`, `--surface-hover`, `--surface-selected`, `--border-strong`)
- No runtime theme toggle; `localStorage.theme` is normalized to `dark`
...
| Component | Purpose |
|-----------|---------|
| `DashboardLayout.vue` | Dark-only shell: sidebar, mobile nav, locale controls |
...
| `SummaryMetric.vue` | Summary-first workspace metric block |
| `EntityInspector.vue` | Read-only selected entity inspector with active border |
| `ImpactConfirmDialog.vue` | Destructive action confirmation with impact summary |
...
| `lib/darkTheme.js` | Dark-only bootstrap helper |
