...
- Pro adds Request and Operation modes. Operation mode exposes the production Tenant Admin and Client menu roots with the documented `0` cancel, `8` next, and `9` back semantics. The Client Console is read-only; the Tenant Admin console lists, creates, edits, activates/deactivates, and deletes local Clients, lists, creates, renames, and deletes Services and Plans, and manages Subscriptions (list/filter, create, edit, cancel, renew, reactivate, reveal) through the same Demo data-source repositories used by the web pages.
- Tenant Admin delete confirmations render current relation previews before mutation. Successful local mutations invalidate the catalog store and refresh the simulator workspace summary, so Clients, Catalog, dependent selectors, and dashboard reads observe the same browser-local records without backend requests. Repository validation and conflicts leave the draft/confirmation flow active and return localized retry prompts.
- Conversation state, form input, generated code, and timers remain component state. Reset Conversation clears only chat, unmount cleanup cancels pending effects, and no simulator action calls WhatsApp, mailbox, n8n, Evolution, or business APIs.
- Starter intentionally exposes Request only; Pro exposes Request and the documented operation menu roots. Subscription validation, lifecycle confirmations, masked credentials, and local relation choices remain aligned with the production-visible contracts while unrelated operation families remain follow-up work.
