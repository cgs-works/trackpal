...
- Pro adds Request and Operation modes. Operation mode exposes the production Tenant Admin menu roots and a read-only Client Console simulation with the documented `0` cancel, `8` next, and `9` back semantics. The Client Console selects current local Clients, renders local profile and active-subscription views (including inactive, empty, expired, and paginated states), and reuses the local access-code request flow. It never creates identities, authenticates a Client portal, mutates business records, or issues business HTTP requests.
