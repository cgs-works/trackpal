...
│   ├── style.css                 # Tailwind CSS v4 entry + custom theme CSS variables (light/dark via oklch)
│   │
│   ├── lib/
...
│   │   └── __tests__/
│   │       └── useTheme.spec.js  # Theme composable tests
│   │
│   ├── test-utils/
│   │   └── renderWithApp.js      # Test helper: mounts components with Pinia + Router for integration tests
│   │
│   ├── components/
│   │   ├── DashboardLayout.vue           # Shared shell: sidebar, mobile nav, theme/locale controls, user card
...
| Theme | `composables/useTheme.js` | Dark/light theme detection, toggling, and localStorage persistence |
| UI Primitives | `components/ui/*` | shadcn-vue components built on Reka UI + Tailwind CSS v4 |
| Shared Components | `components/*.vue` | `DashboardLayout`, `PageHeader`, `InlineAlert`, `StatusBadge`, `EmptyState`, `LoadingBlock`, `ThemeToggle` |
